...
#include "GranularTexturePlugin.h"
#include "../SoundEnginePlugin/GranularTextureFXFactory.h"

GranularTexturePlugin::GranularTexturePlugin()
{
}
...
AK_DEFINE_PLUGIN_CONTAINER(GranularTexture);											// Create a PluginContainer structure that contains the info for our plugin
AK_EXPORT_PLUGIN_CONTAINER(GranularTexture);											// This is a DLL, we want to have a standardized name
AK_ADD_PLUGIN_CLASS_TO_CONTAINER(                                             // Add our CLI class to the PluginContainer
    GranularTexture,        // Name of the plug-in container for this shared library
    GranularTexturePlugin,  // Authoring plug-in class to add to the plug-in container
    GranularTextureFX       // Corresponding Sound Engine plug-in class
);
