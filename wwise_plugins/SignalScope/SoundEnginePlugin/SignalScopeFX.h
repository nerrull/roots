...
#ifndef SignalScopeFX_H
#define SignalScopeFX_H
...
private:
    // (Re)opens m_scopeWriter for the current NonRTPC.iScopeId / format, closing
    // any previous binding. Cheap to call defensively -- Open() itself no-ops
    // most of the work when the slot's already the right shape.
    void SyncScopeWriter();

    SignalScopeFXParams* m_pParams;
    AK::IAkPluginMemAlloc* m_pAllocator;
    AK::IAkEffectPluginContext* m_pContext;

    mi::scope::ScopeWriter m_scopeWriter;
    AkInt32 m_iBoundScopeId;
    char m_szBoundName[kMaxNameLen];
    AkUInt32 m_uSampleRate;
    AkUInt32 m_uNumChannels;
    AkUInt32 m_uBoundChannels;
    bool m_bScopeBound;

    // Short speaker labels ("FL"/"FR"/"LFE"/...) derived from in_rFormat's
    // channel config at Init() time, handed to the shared-memory ring so the
    // monitor app can show them instead of bare channel indices.
    char m_channelNames[mi::scope::kMaxChannelsSupported][mi::scope::kMaxChannelNameLen];
    const char* m_channelNamePtrs[mi::scope::kMaxChannelsSupported];
};

#endif // SignalScopeFX_H
