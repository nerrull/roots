...
void main() {
    ...
    // dim ambient/global directional term -- just enough to keep faces from
    // going fully black when nothing else is lighting them.
    vec3 l = normalize(u_lightDir);
    float lam = abs(dot(n, l));
    vec3 col = v_color * (0.12 + 0.20 * lam);
    ...
    col += v_color * ndotl * atten * 1.6 + vec3(spec) * atten;
